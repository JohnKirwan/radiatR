...
SPEC_GROUP_KEY       <- "__group__"   # "Follow group" colour sentinel
SPEC_CRIT_COLOUR     <- "firebrick"   # Rayleigh critical circle
SPEC_CRIT_LWD        <- 0.7
SPEC_VTEST_MU0       <- pi / 2        # V-test hypothesised direction (display top)
# add_circular_boxplot() geometry defaults, mirrored so the app can push the
# numeric angle labels clear of stacked group rings (keep in sync with the
# package defaults in R/circular_boxplot.R).
SPEC_BOX_RADIUS       <- 1.1
SPEC_BOX_WIDTH        <- 0.05
SPEC_BOX_GROUP_GAP    <- 0.06
SPEC_BOX_LABEL_MARGIN <- 0.16         # clearance beyond the outermost ring
...
build_plot_spec <- function(ts, hd, method, data, inputs) {
  ...
  # Analytical grouping key: a single categorical column (or NULL), validated to
  # be a column of df. Distinct from group_col = id_col (ggplot's path key).
  group_by <- if (!is.null(inputs$group_by) && nzchar(inputs$group_by) &&
                  inputs$group_by %in% names(df)) inputs$group_by else NULL

  cb <- inputs$colour_by
  # "Follow group" resolves to the Group column (trajectory/single when unset).
  if (identical(cb, SPEC_GROUP_KEY)) cb <- group_by %||% SPEC_TRAJ_KEY
  ...
  # Follow-group regime: overlays subdivide + colour-match iff the resolved
  # colour column IS the analytical key (Follow group, or the group col chosen
  # directly). Otherwise Group drives the stats table only.
  follows_group <- !by_traj && !is.null(group_by) && identical(cb, group_by)
  ...
  list(
    ...
    group_col = id_col,
    group_by  = group_by,
    # Distinct group count in the Follow-group regime, used only to push the
    # angle labels clear of the per-group boxplot rings (NULL otherwise). Stored
    # on the spec so spec_to_code reproduces the push without the source data.
    group_n   = if (follows_group && !is.null(group_by))
                  length(unique(stats::na.omit(df[[group_by]]))) else NULL,
    facet_by   = gc,
...
# Corner-label radius that clears N stacked group boxplot rings, or NULL when no
# push is needed (boxplot off, no group, or a single group). Single source of
# truth shared by spec_to_plot (render) and spec_to_code (export) so both push
# the angle labels to the identical radius, keeping the round-trip byte-exact.
.boxplot_label_outer <- function(spec) {
  n <- spec$group_n
  if (!isTRUE(spec$show$boxplot) || is.null(n) || n <= 1L) return(NULL)
  SPEC_BOX_RADIUS + (n - 1L) * SPEC_BOX_GROUP_GAP + SPEC_BOX_WIDTH / 2 +
    SPEC_BOX_LABEL_MARGIN
}
...
spec_to_plot <- function(spec, ts, hd) {
  ...
  by   <- spec$colour$by
  cap  <- spec$colour$cap
  # Follow-group regime: overlays subdivide + colour-match by the analytical key.
  # Otherwise overlay_group is NULL -> today's pooled-per-facet overlays. Computed
  # early because it also decides whether group_by must be attached onto hd below.
  follows_group <- isTRUE(spec$colour$follows_group)
  overlay_group <- if (follows_group) spec$group_by else NULL
  headings_mode <- identical(spec$mode, "headings")
  # Push numeric angle labels clear of stacked group boxplot rings. Derived from
  # spec$group_n (set in build_plot_spec) via .boxplot_label_outer() so the
  # rendered figure and the emitted script push labels identically; NULL when no
  # push is needed. Must be computed before both radiate() calls below.
  boxplot_outer <- .boxplot_label_outer(spec)
  ...
  if (headings_mode) {
    ...
    p <- radiate(
      ...
      angle_label_outer_radius = boxplot_outer,
  ...
  if (!headings_mode) {
    ...
    facet_attach <- setdiff(c(spec$facet_by, spec$facet_cols,
                              if (follows_group) spec$group_by), names(hd))
  ...
  if (spec$show$arrow) {
    arrow_df <- compute_circ_mean(hd, facets = c(spec$facet_by, spec$facet_cols),
                                  group_col = overlay_group, axial = isTRUE(spec$axial))
    p <- p + add_circ_mean(arrow_df,
               colour_col = if (follows_group) spec$group_by else NULL,
               colour     = if (follows_group) NULL else "black",
               axial = isTRUE(spec$axial))
  }
  ...
  if (headings_mode) {
    if (identical(spec$data$source, "example")) {
      ...
      fcols <- unique(c(spec$facet_by, spec$facet_cols,
        if (isTRUE(spec$colour$follows_group)) spec$group_by))
      if (length(fcols))
...
spec_to_code <- function(spec) {
  ...
  grid_mode <- !is.null(spec$facet_cols)
  # Mirror spec_to_plot's overlay_group: emit group_col only in the Follow-group
  # regime, so pooled/default specs emit byte-identical calls.
  follows_group <- isTRUE(spec$colour$follows_group)
  og <- if (follows_group) spec$group_by else NULL
  gcarg <- if (!is.null(og)) paste0(", group_col = ", q(og)) else ""
  # Angle-label push for stacked group boxplot rings; same source of truth as the
  # rendered figure (.boxplot_label_outer) so the emitted radiate() call matches.
  bpo   <- .boxplot_label_outer(spec)
  bparg <- if (!is.null(bpo)) paste0(", angle_label_outer_radius = ", bpo) else ""
  ...
  if (has_hd && spec$show$arrow) {
    add("")
    add("arrow_df <- compute_circ_mean(hd", emit_facets(), gcarg, ax, ")")
  }
  ...
  if (headings_mode) {
    add("radiate(hd, show_markers = FALSE, colour_col = \".colour\"", pby,
        ...
        ", theme = ", q(spec$theme),
        ", angle_labels = ", q(spec$angle_labels), qr, bparg,
        ", display = disp)")
  } else {
    ...
    add("radiate(ts, group_col = ", q(spec$group_col),
        ...
        ", legend = ", if (spec$colour$legend) "TRUE" else "FALSE",
        ", theme = ", q(spec$theme),
        ", angle_labels = ", q(spec$angle_labels), qr, clp, bparg,
  ...
  if (has_hd && spec$show$arrow)
    tail <- c(tail, if (follows_group)
      paste0("add_circ_mean(arrow_df, colour_col = ", q(spec$group_by), ax, ")")
    else
      paste0("add_circ_mean(arrow_df, colour = \"black\"", ax, ")"))
  ...
  if (has_hd && isTRUE(spec$show$ci))
    tail <- c(tail, paste0(
      "add_heading_interval(hd", emit_facets(), gcarg,
      ", stat = \"bootstrap_ci\"", ax, ")"))
  if (has_hd && isTRUE(spec$show$rayleigh))
    tail <- c(tail, paste0(
      "add_critical_r(hd, test = \"rayleigh\"", emit_facets(), gcarg,
  ...
  if (has_hd && isTRUE(spec$show$vtest))
    tail <- c(tail, paste0(
      ...
      gcarg, ", colour = ", q(SPEC_VTEST_COLOUR), ", linewidth = ", SPEC_VTEST_LWD, ax, ")"))
  if (has_hd && isTRUE(spec$show$boxplot))
    tail <- c(tail, paste0("add_circular_boxplot(hd", ax,
                           ", theme = ", q(spec$theme %||% "void"), emit_facets(), gcarg, ")"))
