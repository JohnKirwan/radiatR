...
# Internal: inverse of .uc_angle_to_display(). Convert a display value back to
# a UC angle (radians, [0, 2*pi)). Vectorised over `val`.
.display_to_uc_angle <- function(val, display = circ_display()) {
  val_rad <- if (display$units == "degrees") val * pi / 180 else val
  if (display$clockwise) {
    (display$zero - val_rad) %% (2 * pi)
  } else {
    (val_rad + display$zero) %% (2 * pi)
  }
}
...
#' @param position Where to place the labels: `"outside"` (default) keeps the
#'   four diagonals (45/135/225/315) in the plot corners, as before;
#'   `"inside"` puts all eight directions (including the cardinals
#'   0/90/180/270) just inside the circle at `inside_radius`; `"split"` puts
#'   the cardinals inside and the diagonals outside. The `"inside"`/`"split"`
...
#' @param outer_radius Optional corner magnitude for the diagonal `"outside"`
#'   labels. `NULL` (default) keeps the historical `0.9*sqrt(2)` distance; a
#'   larger value pushes them outward (e.g. to clear stacked circular-boxplot
#'   rings).
...
degree_labs <- function(display = circ_display(), colour = "black",
                        ...
                        inside_radius = 0.88, n = NULL, outer_radius = NULL,
                        color = NULL) {
  .apply_spelling_aliases()
  ...
  fmt <- function(deg) if (units == "radians") .format_pi_deg(deg)
                       else paste0(deg, "\U00B0")
  lab_layer <- function(x, y, lab)
    ggplot2::annotate("text", x = x, y = y, label = lab, colour = colour,
                      size = size, family = family)
  outer_r <- if (is.null(outer_radius)) 0.9 * sqrt(2) else outer_radius  # corner distance; default clears a boxplot band (~1.13)
  at_r <- function(deg, r) {
    th <- deg * pi / 180
  ...
  if (position == "outside") {
    diag_r <- outer_r / sqrt(2)            # corner x/y component; clears the boxplot band
    pos    <- list(c(diag_r,  diag_r), c(diag_r, -diag_r),
                   c(-diag_r, -diag_r), c(-diag_r,  diag_r))
    diag_d <- c(45, 135, 225, 315)
    return(mapply(function(p, deg) lab_layer(p[1], p[2], fmt(deg)),
  ...
  diag_layers <- lapply(diagonals, function(deg) {
    xy <- at_r(deg, if (position == "inside") inside_radius else outer_r)
    lab_layer(xy$x, xy$y, fmt(deg))
  })
  c(card_layers, diag_layers)
}
  ...
  if (angle_labels != "none") {
    if (is.null(scale))
      g <- g + degree_labs(display = display, units = angle_labels,
                           ...
                           outer_radius = angle_label_outer_radius)
    else
...
#' @param angle_label_outer_radius Optional corner magnitude for the numeric
#'   diagonal labels (`"degrees"`/`"radians"`). `NULL` (default) keeps the
#'   standard position; a larger value pushes the labels outward to clear
#'   wide/stacked circumference overlays such as grouped circular boxplots.
...
function(
  ...
  g <- .radial_chrome_foreground(
    ...
    angle_label_outer_radius = angle_label_outer_radius,
    n_labels = n_labels
  )
...
#'   present, otherwise the identity display.
#' @param axial Logical; when `TRUE`, draw the axial (bidirectional) density of an
#'   axial fit (from `wrappedcauchy_fit(axial = TRUE)`): the curve is evaluated on
#'   doubled angles, giving two equal peaks at `mu` and `mu + pi`. Default
#'   `FALSE`.
#' @return A \code{geom_polygon} layer, or \code{NULL} if estimation fails.
