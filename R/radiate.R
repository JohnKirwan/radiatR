...
if(ticks==TRUE){tick_df <- data.frame(
  ...
  ggplot2::ggplot(data=data) + ggplot2::coord_fixed() +
    ggplot2::geom_path(data = data,
      ...
      x=.data$rel_x,
      y=.data$rel_y,
      color=ggplot2::.data$id))+
    ggplot2::annotate("path",colour="black",x=cos(seq(0,2*pi,0.06)),y=sin(seq(0,2*pi,0.06)))+
    ggplot2::annotate("path", color="blue",linetype="dashed",
   ...
   if(is.null(group1)==FALSE){
     ggplot_obj <- ggplot_obj + ggplot2::facet_wrap(.~get(group1),ncol=ncols,
                                         strip.position="top")
   }
  return(ggplot_obj)
}
