...
#' @param group_col Optional single column name in `hd`. When set, one boxplot is
#'   drawn per group level as a concentric ring stepped outward from `radius` by
#'   `group_gap`, with the box outline, whiskers, crossbars and far-out points
#'   coloured by the group. `NULL` (default) draws a single boxplot. Combine with
#'   `facets` to subdivide each panel. A group with fewer than 4 usable
#'   observations (or a non-unique median) is skipped with a warning.
#' @param group_gap Radial step between consecutive group rings. Default `0.06`.
...
add_circular_boxplot <- function(hd, angle_col = "heading", axial = FALSE,
                                 ...
                                 facets = NULL, group_col = NULL,
                                 group_gap = 0.06, theme = NULL, color = NULL) {
  ...
  build_geom <- function(s, ring_radius = radius, gid_base = 0L) {
    r_in  <- ring_radius - width / 2
    r_out <- ring_radius + width / 2
    M   <- s$median
    ...
    for (k in seq_along(offs)) {
      ...
      outer <- arc_xy(base + h[1], base + h[2], r_out, gid)
      inner <- arc_xy(base + h[2], base + h[1], r_in,  gid)
      box_df <- rbind(box_df, outer, inner)
      # whiskers: hinge -> whisker end on each side, along the circumference
      whisk_df <- rbind(whisk_df,
        arc_xy(base + h[2], base + w[2], ring_radius, 2L*gid - 1L),
        arc_xy(base + w[1], base + h[1], ring_radius, 2L*gid))
      ...
      if (show_median_arrow) arrow_df <- rbind(arrow_df, radial_xy(base, 0, ring_radius))
      if (length(fo)) {
        xy <- .uc_to_display_coords(ring_radius * cos(base + fo), ring_radius * sin(base + fo), display)
  ...
  # One boxplot, or one per facet/group cell when `facets`/`group_col` names
  # column(s) of hd.
  grouped <- !is.null(group_col) && is.data.frame(hd) && group_col %in% names(hd)
  ...
  if (!faceted && !grouped) {
    ...
    # group level order: factor levels if a factor, else sorted uniques
    glev <- if (grouped) {
      gv <- hd[[group_col]]
      if (is.factor(gv)) levels(droplevels(gv)) else sort(unique(gv[!is.na(gv)]))
    } else NULL
    ...
    for (ci in seq_along(cells)) {
      ...
      if (!is.na(s$reason))
        warning("add_circular_boxplot [", lv, "]: ", s$reason, call. = FALSE)
      k <- if (grouped) match(cell$keys[[group_col]], glev) - 1L else 0L
      g <- build_geom(s, ring_radius = radius + k * group_gap, gid_base = 4L * ci)
  ...
  box_map   <- ggplot2::aes(x = .data$.x, y = .data$.y, group = .data$.g)
  line_map  <- ggplot2::aes(x = .data$.x, y = .data$.y, group = .data$.g)
  cross_map <- ggplot2::aes(x = .data$.x, y = .data$.y, xend = .data$.xend, yend = .data$.yend)
  pt_map    <- ggplot2::aes(x = .data$.x, y = .data$.y)
  if (grouped) {
    gsym <- rlang::sym(group_col)
    box_map[["colour"]]   <- gsym
    line_map[["colour"]]  <- gsym
    cross_map[["colour"]] <- gsym
    pt_map[["colour"]]    <- gsym
  }
  stroke <- function(args) if (grouped) args else c(args, list(colour = colour))

  layers <- list(
    do.call(ggplot2::geom_polygon, stroke(list(data = box_df, mapping = box_map,
      fill = box_fill, linewidth = linewidth, alpha = 0.6, inherit.aes = FALSE))),
    do.call(ggplot2::geom_path, stroke(list(data = whisk_df, mapping = line_map,
      linewidth = linewidth, inherit.aes = FALSE))),
    do.call(ggplot2::geom_segment, stroke(list(data = cross_df, mapping = cross_map,
      linewidth = linewidth, inherit.aes = FALSE)))
  )
  if (show_median_arrow)
    layers <- c(layers, list(do.call(ggplot2::geom_segment, stroke(list(
      data = arrow_df, mapping = cross_map, linewidth = linewidth,
      arrow = grid::arrow(length = grid::unit(0.2, "cm")), inherit.aes = FALSE)))))
  if (!is.null(fo_df))
    layers <- c(layers, list(do.call(ggplot2::geom_point, stroke(list(
      data = fo_df, mapping = pt_map, shape = farout_shape, inherit.aes = FALSE)))))
  layers
}
