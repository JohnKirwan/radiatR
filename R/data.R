...
#' without an obstacle.
#'
#' Coordinates are normalised to the unit circle (arena radius = 1).
#' An `arc` column (ordered factor) encodes the stimulus arc angle in
#' degrees.
#'
#' The corresponding raw landmark and track text files for the five
#' arc = 0° baseline trials are shipped in `inst/extdata/tracks/` to
...
#'
#' @format A [`TrajSet`] object with 134 375 rows and the following data
#'   columns:
#'   \describe{
#'     \item{trial_id}{Character. Unique trial identifier (folder/filename).}
...
#'   }
#'
#' @source Experiment conducted at the University of Exeter. Raw tracking
