...
#'
#' Pre-processed `TrajSet` of 263 millipede trajectories from a visual-acuity
...
#' conditions were tested: target half-widths of 5, 10, 15, 20, 30, 40, and
#' 50 degrees, plus a featureless control (`arc = 0`). Some trials lack a
#' manifest entry and so have `arc = NA`.
#'
...
#' and `rel_y` give the target-relative heading and position.
#'
...
#'
#' @format A [`TrajSet`] object (50,885 observations) whose data columns include:
#'   \describe{
...
#'       `NA` for trials without a manifest entry.}
...
#'   produced with dtrack and normalised with \pkg{radiatR}.
"cpunctatus"

#' *Cylindroiulus punctatus* trajectory tibble (millipede tracks)
#'
#' A tidy long-form tibble (50,885 rows) holding the per-frame observations
