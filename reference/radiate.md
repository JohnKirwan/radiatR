...
- angle_label_position:
  ...
  labels via \[circumference_labs()\]. See \[degree_labs()\].

- angle_label_outer_radius:

  Optional corner magnitude for the numeric diagonal labels
  (\`"degrees"\`/\`"radians"\`). \`NULL\` (default) keeps the standard
  position; a larger value pushes the labels outward to clear
