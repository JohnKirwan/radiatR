...
Adds perimeter text labels (cardinal directions, hours, months, seconds,
or a custom scale) to a radial ggplot. The display-aware sibling of
\[degree_labs()\]: labels are positioned through the supplied
...
- scale:
  ...
  A perimeter-scale list, from \[scale_cardinal()\], \[scale_clock()\],
  \[scale_months()\], \[scale_seconds()\], or hand-written as \`list(n
  =, at =, labels =)\` (\`at\` in raw unit-circle radians, 0 = East,
