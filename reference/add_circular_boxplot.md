...
- group_col:
  ...
  Optional single column name in \`hd\`. When set, one boxplot is drawn
  per group level as a concentric ring stepped outward from \`radius\`
  by \`group_gap\`, with the box outline, whiskers, crossbars and
  far-out points coloured by the group. \`NULL\` (default) draws a
  single boxplot. Combine with \`facets\` to subdivide each panel. A
  group with fewer than 4 usable observations (or a non-unique median)
  is skipped with a warning.

- group_gap:

  Radial step between consecutive group rings. Default \`0.06\`.
