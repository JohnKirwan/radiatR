...
# ---- .append_transform_history / log_transform: empty ids --------------------

test_that(".append_transform_history is a no-op for empty ids", {
  h0 <- radiatR:::.empty_transform_history()
  # default params/depends_on
  h1 <- radiatR:::.append_transform_history(
    h0, step = "s", ids = character(0), implementation = "s", params = NULL)
  expect_equal(nrow(h1), 0L)
  # a scalar param (as restrict_to_circumference passes) must not error either
  h2 <- radiatR:::.append_transform_history(
    h0, step = "s", ids = character(0), implementation = "s", params = list(a = 1))
  expect_equal(nrow(h2), 0L)
})

test_that("log_transform with no surviving ids leaves the history unchanged", {
  ts  <- mk_track(c(0, 0.3, 0.5))
  h0  <- transform_history(ts)
  out <- log_transform(ts, step = "noop", traj_ids = character(0))
  expect_equal(transform_history(out), h0)
})
