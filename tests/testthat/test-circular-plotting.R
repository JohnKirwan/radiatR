...
test_that("degree_labs outer_radius pushes the outside diagonal labels", {
  # default: historical corner x/y component 0.9 (magnitude 0.9*sqrt(2))
  d0 <- degree_labs(position = "outside")[[1]]
  expect_equal(abs(d0$data$x), 0.9)

  # set: x/y component scales as outer_radius / sqrt(2)
  d1 <- degree_labs(position = "outside", outer_radius = 1.5)[[1]]
  expect_equal(abs(d1$data$x), 1.5 / sqrt(2))
})

test_that("radiate angle_label_outer_radius pushes the numeric corner labels", {
  d0 <- data.frame(id = "a", t = 1:4,
                   rel_x = c(0.1, 0.2, 0.3, 0.4), rel_y = c(0, 0.1, 0.2, 0.1),
                   x = c(0.1, 0.2, 0.3, 0.4), y = c(0, 0.1, 0.2, 0.1))
  ts <- tracks(d0, id = "id", time = "t",
               x = "x", y = "y", rel_x = "rel_x", rel_y = "rel_y",
               normalize_xy = FALSE)
  p0 <- radiate(ts, angle_labels = "degrees", show_arrow = FALSE)
  p1 <- radiate(ts, angle_labels = "degrees", angle_label_outer_radius = 1.6,
               show_arrow = FALSE)
  # locate the diagonal text annotation layers and compare their corner magnitude
  corner <- function(p) {
    xs <- unlist(lapply(p$layers, function(l)
      if (!is.null(l$data) && all(c("x", "y") %in% names(l$data)) &&
          inherits(l$geom, "GeomText")) max(abs(l$data$x)) else NA_real_))
    max(xs, na.rm = TRUE)
  }
  expect_gt(corner(p1), corner(p0))
})
