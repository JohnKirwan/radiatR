...
roundtrip_spec <- function(heading_display, by, facet, arrow, vectors,
                           ...
                           group_by = NULL, follows_group = FALSE) {
  ...
  # Attach the analytical group column onto hd (as the app does) so a grouped
  # boxplot renders per-group rings; compute the distinct group count for the
  # angle-label push, mirroring build_plot_spec's group_n.
  group_n <- NULL
  if (follows_group && !is.null(group_by)) {
    df <- as.data.frame(ts)
    hd[[group_by]] <- df[[group_by]][match(hd$id, df[[ts@cols$id]])]
    group_n <- length(unique(stats::na.omit(hd[[group_by]])))
  }
  spec <- list(
    data = list(source = "example", path = NULL, dialect = NULL),
    headings = list(rule = "distal"),
    group_col = ts@cols$id, group_by = group_by, group_n = group_n,
    facet_by = facet, facet_cols = facet_cols,
...
test_that("grouped circular boxplot round-trips (per-group rings + label push)", {
  rt <- roundtrip_spec("points", by = "type", facet = NULL,
                       arrow = FALSE, vectors = FALSE, boxplot = TRUE,
                       group_by = "type", follows_group = TRUE)
  code <- spec_to_code(rt$spec)
  # emits group_col on the boxplot and the pushed angle-label radius on radiate()
  expect_match(code, 'add_circular_boxplot(hd, theme = "void", group_col = "type")',
               fixed = TRUE)
  expect_match(code, "angle_label_outer_radius = ", fixed = TRUE)
  suppressWarnings(expect_roundtrip(rt))
})
...
# --- Group-by (Follow-group regime) round-trips --------------------------------
# by = "type" with follows_group = TRUE mimics Colour = "Follow group" resolved
# to the `type` column: marks colour by type and the overlays colour-match, all
# subdivided by group_col = "type".
test_that("round-trip: Follow-group Rayleigh + arrow + CI (group set)", {
  rt <- roundtrip_spec("points", "type", NULL, TRUE, FALSE,
                       rayleigh = TRUE, ci = TRUE,
                       group_by = "type", follows_group = TRUE)
  expect_roundtrip(rt)
})

test_that("round-trip: group set but colour = trajectory -> pooled overlays", {
  rt <- roundtrip_spec("points", "trajectory", NULL, TRUE, FALSE,
                       rayleigh = TRUE,
                       group_by = "type", follows_group = FALSE)
  expect_roundtrip(rt)
})

test_that("stats code groups by group_by when set", {
  rt <- roundtrip_spec("points", "type", NULL, FALSE, FALSE,
                       group_by = "type", follows_group = TRUE)
  rt$spec$stats <- list(by_col = "type", pooled = FALSE,
                        omnibus = "rao", axial = FALSE)
  code <- spec_to_stats_code(rt$spec)
  expect_match(code, "\\.by = \"type\"")
  expect_match(code, "circ_model_select\\(hd, group_col = \"type\"\\)")
})

test_that("round-trip: headings-example Follow-group attaches the group col (emit runs)", {
  # Regression: the headings-example preamble must attach the group column, not
  # only the facet, or the emitted overlays reference a column absent from hd.
  data(cpunctatus, package = "radiatR", envir = environment())
  ts <- cpunctatus
  hd <- derive_headings(ts, rule = "distal", coords = "relative")
  df <- as.data.frame(ts); idc <- ts@cols$id
  cond <- unique(df[, c(idc, "arc", "type")])
  hd <- merge(hd, cond, by.x = "id", by.y = idc, all.x = TRUE)
  hd <- build_headings_input(hd, col = "heading", units = "radians",
                             convention = "unit_circle")
  spec <- build_plot_spec(
    ts = NULL, hd = hd, method = NULL,
    data = list(source = "example", mode = "headings", path = NULL),
    inputs = list(cond_col = "arc", group_by = "type", colour_by = "__group__",
                  plot_theme = "void", angle_labels = "degrees",
                  heading_display = "points", omnibus_test = "rao",
                  show_arrow = TRUE, show_rayleigh = TRUE, show_ci = TRUE))
  expect_true(spec$colour$follows_group)     # facet = arc, group = type
  expect_equal(spec$group_by, "type")
  code <- spec_to_code(spec)
  expect_match(code, "\"type\"", fixed = TRUE)   # group col attached in preamble
  set.seed(1); live  <- spec_to_plot(spec, NULL, hd)
  env <- new.env(parent = globalenv())
  set.seed(1); evald <- eval(parse(text = code), envir = env)   # must run, not error
  expect_equal(.fingerprint(evald), .fingerprint(live))
})
