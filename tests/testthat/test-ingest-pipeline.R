test_that("file-based ingest pipeline produces normalised tracks", {
  ...
  track_objects <- get_tracked_object_pos(trial_limits, tracks_df)
  expect_type(track_objects, "list")
  expect_equal(length(track_objects[[1]]), 2)
  expect_true(all(track_objects[[2]]$valid_track))

  combined <- get_all_object_pos(landmarks_df, tracks_df, file_tbl_aug, tmp_dir)
  expect_s3_class(combined[[1]], "data.frame")
  expect_gt(nrow(combined[[1]]), 0)
  expect_true(all(abs(combined[[1]]$rel_x) <= 1.5))
})
