...
for. The axial estimator `velocity_axis` recovers the axis, while a directional
rule (`net`) sees the to-and-fro directions cancel:

```{r oscillatory-axis}
osc <- tibble::tibble(condition = "osc", n_trials = 40L, ref_mean = 0.6,
                      concentration_base = 50, track_shape = "oscillatory",
                      n_reversals = 4L)
ts_osc <- simulate_tracks(conditions = osc, n_points = 80,
                          output = "trajset", seed = 11)

# velocity_axis recovers the ~0.6 rad axis (read it as an axial mean)
va <- derive_headings(ts_osc, rule = "velocity_axis")
circ_summarise(va, "heading", axial = TRUE, units = "radians",
               stats = "mean_dir")
...
settings used here without any signal-to-noise tuning.
