...
`simulate_tracks()` generates synthetic circular-arena trajectories without any
tracking files. Its main uses are:
...
| `stim_mean` | `0` | Baseline stimulus direction (unit-circle radians) |
...
conds_kappa <- data.frame(
  condition          = c("low (κ=1)", "medium (κ=4)", "high (κ=12)"),
  n_trials           = 20L,
  concentration_base = c(1, 4, 12),
...
## Directional Bias (stim_mean)

`stim_mean` shifts the stimulus direction away from East (0 rad). Use it to
simulate experiments where the stimulus is not along the positive x-axis.
...
A common design is a monotone gradient in stimulus strength. The example below
simulates four levels with increasing concentration and decreasing tortuosity —
mimicking an orientation experiment where stronger stimuli produce tighter,
smoother paths.
...
compute_circ_mean(hd, colour_col = "condition")[,
  c("condition", "mean_dir", "resultant_R", "n")]
```
